...
function M_Main_Draw() {
	...
	// If in-game, showing Continue option
	if (inGame) {
		M_Print(72, y, "Continue");
		y += 20;
	}

	M_Print(72, y, "Single Player");
	M_Print(72, y + 20, "Multiplayer");
	M_Print(72, y + 40, "Options");
	M_Print(72, y + 60, "Help/Ordering");
	M_Print(72, y + 80, "Leaderboard");
	M_Print(72, y + 100, "Quit");

	// Cursor
	const f = Math.floor(_host_time_get() * 10) % 6;
	const cursorIndex = inGame ? m_main_cursor : m_main_cursor; // logic handled in Touch/Key, here we just draw based on cursor

	// If inGame, there is 1 extra item at the top.
	// But m_main_cursor usually iterates 0..N-1.
	// We need to map cursor to screen Y.

	// If inGame: items are 0..6 (7 items total). 0=Continue, 1=Single, ...
	// If !inGame: items are 0..5 (6 items). 0=Single, ...

	let cursorY = 32 + m_main_cursor * 20;

	M_DrawTransPic(54, cursorY, _Draw_CachePic('gfx/menudot' + (f + 1) + '.lmp'));
		...
		case K_ENTER:
			...
			switch (actionCursor) {
				...
				case 4:
					...
					// We stay in the menu, but the modal overlays it
					break;
				case 5:
